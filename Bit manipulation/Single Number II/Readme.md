...
> When every element appears **3 times except one**, the Bucket Method (`ones` and `twos`) is the best solution because it processes every bit in constant space while maintaining linear time complexity.